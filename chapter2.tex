...
Constraints acting on the configuration, i.e. positions and orientations, of a system of rigid bodies are said to be  \textit{holonomic}. 
Holonomic constraints are expressed in the form:
\begin{equation}
h\left( x\right) =0
\end{equation}
Constraints on the velocities of the system of rigid bodies but not on their positions are called \textit{nonholonomic}. In general all constraints which are nonintegrable are said to be nonholonomic, so also inequality constraints about the configuration are considered nonholonomic. 
...
\end{equation}
Where $x\in \mathbb{R}^n $ is the state vector, $u= \left[
\begin{matrix}
u_1 &  \cdots & u_m
\end{matrix}
...
\end{figure}
A unicycle is a vehicle which configuration is completely described by the state vector $x = \left[\begin{matrix}x_b&y_b&\theta_b\end{matrix}\right]^T$, where $x_b$ and $y_b$ are the Cartesian coordinates of the contact point of the wheel with the ground and $\theta_b$ is the orientation of the wheel with respect to the axis of the abscissas. \\
...
\end{equation}
Where $A_i^{i-1}$ is the expression of the transformation from one joint frame to the following one through the use of Denavit-Hartemberg parameters \cite{DH}: $a\text{, }\alpha\text{, }d\text{, }\theta$
\begin{equation}
A_i^{i-1}(q_i)=\left[
\begin{matrix}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.4]{denavit_hartenberg}
...
\begin{equation}\label{dirkinMM}
	X_{EE}=\left[\begin{matrix}\mathbf{p}_{EE}\\\Phi\end{matrix}\right]=f(x)=f\left(\left[\begin{matrix}x_{base}\\x_{arm} \end{matrix}\right]\right)
\end{equation}
As an example, the forward kinematics for just the Cartesian coordinates of the end effector of the simplified system in figure \ref{MM_kin} is
\begin{equation*}
\begin{split}
...
\begin{equation}\label{eq:dotx_EE}
	\dot{X}_{EE}=J_b(x)\dot{x}_b+J_a(x)\dot{x}_a
\end{equation}
Where the columns of $J_a(x)=\frac{\partial f}{\partial x_a}(x_a,\theta)$ can be obtained as:
\begin{equation}
\mathit{j}_{P_j}^{(EE)}=
	\begin{cases}
...
\begin{equation}
	...
	\end{cases} 
\end{equation}
and $J_b(x)={\partial f}/{\partial x_b}(x)$ can be obtained at the same way considering $x_b$ and $y_b$ as prismatic joints and $\theta_b$ as a revolute joint of the compound system.\\
...
J_v(\theta_b) & J_a(x)
\end{matrix}\right]\left[\begin{matrix}v\\\dot{x}_a \end{matrix}\right] = J(x)\dot{x}
\end{equation}
