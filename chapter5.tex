...
\begin{split}
		& min_{\textbf{u}_k}\ J({x}_{k|i},{u}_{k|i}) \\
		\textnormal{s.t.}\qquad
...
\begin{equation}
	\hat{h}(e_N(e,p^*)) \leq \eta\ c^m\ \ \  \textnormal{where    } c:=\frac{N-1}{N} < 1
 	\label{lemma1}
\end{equation}
...
\begin{equation}
	J(e,p^*)\leq \left(\frac{N-1}{N}\right)\sum_{i=1}^{N-1}\hat{h}(e,p^0)
\end{equation}
and considering:
\begin{equation}
