...
\begin{figure}[p]
	\centering
	\includegraphics[scale=0.47]{control_actions_SIM}
...
Once all the MPC parameters and weights have been properly tuned, the MM has been tested performing a trajectory following task, tracking the position and orientation of the end-effector along a given trajectory in order to grasp an object. In particular, the robot is asked to move towards a grasping area, grasp the object preferably without stopping the mobile base, and then move away from the grasping area. In all these steps the obstacle avoidance and self collision avoidance constraints have to be satisfied, maximizing also the manipulability measure and limiting maximum accelerations and maximum speeds of all joints. At the same time the MPC optimization process solves the kinematic redundancy problem.
The pre-planned desired trajectories of the end effector and mobile base are shown in Figure \ref{ref_traj}. The given trajectories do not involve high speeds for the mobile base for two main reasons: first of all, since the controller has been developed on Matlab/Simulin
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.55]{ref_traj_REAL}
	\caption{Desired trajectories of end effector and mobile base during experiments with the physical system}
	\label{ref_traj}
\end{figure}
Experiments have been performed with $N=10$ and choosing $T=0.4\ s$.
